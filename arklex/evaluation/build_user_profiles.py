...
ATTR_TO_PROFILE = "Convert the following list user attributes in to a text description of a user profile for valorant, which has a summary {company_summary}\nThe user attributes are here:\n{user_attr}"
ADAPT_GOAL = "Assume you are planning to speak to a coach bot with the following goal in mind:\n{goal}\nUsing the company information below, re-write this goal into one that is more specific to the game. The new goal should mention specific aspects of the game (if relevant) or other details about the game. Here is a summary of the game:\n{company_summary}\nHere is a page from the game website:\n{company_doc}"
ADD_ATTRIBUTES = "Your job is to add attributes to a user profile. Here is an example of an existing profile with the categories on the left and the attributes on the right:\n{user_profile}\nSuggest three attributes for the following category:\n{category}\nThese attributes should be specific values that are relevant to the category and apply to potential customers of the company. You should return a comma separated list of attributes without any descriptions of the attributes. Generated the attributes based on a summary of the company and the company webpage and what kind of customers the compnay is likely targeting. Here is the summary fo the company:\n{company_summary}\nHere is the webpage:\n{company_doc}"
